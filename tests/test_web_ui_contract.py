...
def test_frontend_recovers_from_a_stale_version():
    assert 'const CLIENT_APP_VERSION = "0.5.54";' in APP_JS
...
def test_ai_report_supports_typora_style_themes_with_github_default():
    assert 'const AI_REPORT_THEME_STORAGE_KEY = "tpa-ai-report-theme";' in APP_JS
    assert '{ id: "github", label: "GitHub" }' in APP_JS
    assert '{ id: "newsprint", label: "Newsprint" }' in APP_JS
    assert '{ id: "night", label: "Night" }' in APP_JS
    assert ': "github");' in APP_JS
    assert 'class="ai-report-theme-picker"' in APP_JS
    assert "'ai-report-theme-' + aiReportTheme" in APP_JS
    ...
    assert STYLE_CSS.count('--ai-report-font: "Inter", "PingFang SC"') == 2
    assert '--ai-report-font: "Source Serif 4", "Noto Serif SC", "Songti SC"' in STYLE_CSS
    assert STYLE_CSS.count('--ai-report-mono: "SFMono-Regular"') == 3
...
def test_header_exposes_user_group_and_new_trial_entry_points():
    ...
    assert 'const NEW_TRIAL_URL = "http://tpa.cambricon.com/new-tpa-web/";' in APP_JS
    ...
    assert 'src="/static/tpa-user-group.jpeg?v=0.5.54"' in INDEX_HTML
...
def test_first_entry_notice_requires_user_opt_out():
    assert 'v-if="showEntryNotice"' in INDEX_HTML
    assert 'v-model="entryNoticeNeverShow"' in INDEX_HTML
    assert '<span>不再显示</span>' in INDEX_HTML
    assert 'const ENTRY_NOTICE_STORAGE_PREFIX = "tpa-entry-notice-v1";' in APP_JS
    assert 'const entryNoticeNeverShow = ref(false);' in APP_JS
    assert 'entryNoticeNeverShow.value = false;' in APP_JS
    assert '`${ENTRY_NOTICE_STORAGE_PREFIX}:${projectExpansionUserKey()}`' in APP_JS
    assert 'localStorage.setItem(entryNoticeStorageKey(), "true");' in APP_JS
    assert 'watch([authChecked, authRequired, currentUser], maybeShowEntryNotice, { immediate: true });' in APP_JS
    assert '@click="openUserGroupFromEntryNotice"' in INDEX_HTML
    assert '@click="closeEntryNotice">立即试用新版</a>' in INDEX_HTML
