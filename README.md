...
- **实验树**：按项目把多次实验连接成优化谱系，支持 Compute time 优先的节点/关系状态配色、边 delta 芯片、节点/关系拖拽缩放、自动避让、变量变更记录和 step 口径告警。
...
- 节点卡以 Compute time 为主指标，E2E、Kernel 作为次要指标；优化结果优先使用 Compute time 判断，并通过卡片底色、边框和文字色区分性能最优、正优化和负优化。
