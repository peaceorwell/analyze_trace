...
- **区域重分析**：完成任务后可按 step、时间轴区间标签（如 optimizer/module）或两者交集派生新分析；对比任务支持 A/B 分别指定不同区域。
...
- 对比结果可交换 A/B 重新对比。
- 任务详情右上角菜单支持 `指定区域重分析`。单 trace 可填写 step、精确的时间轴区间标签，或同时填写以分析两者的交集；同名标签的多个交集区间按多次样本求平均。对比任务可为 A/B 分别设置，留空的一侧使用完整 trace，提交后会生成一个新任务。
