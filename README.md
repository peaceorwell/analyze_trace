...
- `所有 Kernel`、`Triton`、`ATen Ops`：表格化查看明细；Kernel 通过 `External id`（TensorFlow 使用 `tf_op`）关联 Host/Aten Op，多关联、部分匹配或未匹配项可在 `Host Op 关联数` 列按需展开；`Triton 对比` 会优先用 Triton code 指纹、code signature、多 step 指纹交集和规整化名称匹配 A/B kernel，减少末尾数字后缀不同导致的错位。
...
- 对比结果可交换 A/B 重新对比。
- 任务详情右上角菜单支持 `指定区域重分析`。单 trace 可填写 step、精确的时间轴区间标签，或同时填写以分析两者的交集；组合筛选时，同一源 step 内的多个交集区间汇总为一条 step 结果。对比任务可为 A/B 分别设置，留空的一侧使用完整 trace，提交后会生成一个新任务。
...
- 画布支持整体缩放、平移、自动整理、右侧面板折叠；自动整理会按实际内容收敛节点尺寸，节点支持拖拽排布，关系标签支持拖拽与水平/垂直缩放。
...
单 trace skill 在写出最终报告后会运行 `.claude/skills/e2e-profiling-analyzer/scripts/check_report.py`，以确定性校验替代散文式的报告格式约束：检查章节集合与顺序、2-4 个 `发现` 结构、`结论/证据/建议` 段落形态、表格分隔行、原始 stdout 残留、`Triton Kernel 代码优化` 章节及候选完整性、custom op / simple aten 必报项，以及正文长度预算，未通过时先修报告再收尾。
...
| `step_N_triton_kernels.csv` | 开启 `-s` 后输出的逐 step Triton 明细，`launch_dims` 汇总 trace 中的 `dimx/dimy/dimz` |
