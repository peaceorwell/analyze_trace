...
- For `Custom Op Simple Aten Nesting Delta`, increased nested simple `aten::` count or avg-per-call in current means a custom/user op boundary still dispatches many small pointwise/view/reduce/copy/allocation ops. Cite the custom op name and top nested ops, and recommend moving those simple ops into the custom backend kernel or restructuring the wrapper so Inductor can fuse them.
