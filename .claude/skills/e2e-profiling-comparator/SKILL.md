...
- `scripts/collect_profile_tables.py`: collect host summaries and device breakdown tables for one cnperf DB over one explicit time range. Also emits the device-stream (queue) gap ratio and torch.compile/triton sections: fusion coverage, Inductor fusion granularity by kernel family (pointwise/reduce/library/etc.), compile segmentation + host-launch-overhead (cpp_wrapper signal), custom-op ranges that contain many simple `aten::` ops, and triton kernel IO efficiency (`io_efficiency` as folded bandwidth vs MLU-model theoretical peak).
- `scripts/compare_profile_tables.py`: compare baseline/current table JSON files and emit A/B/delta evidence, including a direction-aware host-overhead delta (device-stream gap, launch self-time), fusion/segmentation deltas, custom-op simple-aten nesting deltas, pointwise/reduce unfused fusion-granularity deltas, and per-kernel IO-efficiency deltas.
...
6. Compare from coarse to fine.
   ...
   - When the workload uses torch.compile/triton, use the fusion-coverage, Inductor fusion-granularity, compile-segmentation, custom-op simple-aten nesting, and triton kernel IO-efficiency deltas to locate fusion loss, unfused pointwise/reduce kernels, custom op wrappers that still run simple `aten::` ops, eager fallback, recompilation, or per-kernel bandwidth regressions.
...
- For `Custom Op Simple Aten Nesting Delta`, increased nested simple `aten::` count or avg-per-call in current means a custom/user op boundary still dispatches many small pointwise/view/reduce/copy/allocation ops. If `must_report=true` or `report_priority_B=high`, include it in `## 结论概览` and `## 优先行动` even when larger time deltas exist elsewhere. Cite the custom op name and top nested ops, and recommend moving those simple ops into the custom backend kernel or restructuring the wrapper so Inductor can fuse them.
