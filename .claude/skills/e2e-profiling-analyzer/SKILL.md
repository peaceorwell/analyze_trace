...
2. `## 结论概览`
   ...
   - If `triton_code_optimization.json` reports `has_findings=true` and `final_report_guidance.promote_to_finding=true`, one finding or one action must explicitly cover the strongest Triton code-level optimization candidate. If it is not material enough for a top finding, it still must appear in `关键指标` or `不确定性与下一步`; do not leave Triton code analysis visible only in `产物`.
...
5. Also scan `triton_code_optimization.json` when present. If `has_findings=true`, read `final_report_guidance` first. Always surface the code-level analysis somewhere user-visible: include a `关键指标` row with scanned file count, candidate kernel count, top strategies, and source file; if `final_report_guidance.promote_to_finding=true` or the strongest candidate has material `total_ms`, low `bandwidth_utilization`, or a repeated pattern, merge it into the Triton IO-efficiency finding or add one focused finding and one action row. If it is not material enough, state in `不确定性与下一步` that Triton code candidates exist but should be validated after larger bottlenecks are fixed. Keep wording as a validation target unless runtime evidence confirms a speedup.
